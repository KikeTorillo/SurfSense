...
import {
	createVideoGenConfigMutationAtom,
	deleteVideoGenConfigMutationAtom,
	updateVideoGenConfigMutationAtom,
} from "@/atoms/video-gen-config/video-gen-config-mutation.atoms";
import {
...
import type { VideoGenerationConfig } from "@/contracts/types/new-llm-config.types";
import { getProviderIcon } from "@/lib/provider-icons";
import { cn } from "@/lib/utils";

// ---------------------------------------------------------------------------
// Video Gen Providers & Models
// ---------------------------------------------------------------------------
...
interface VideoModelManagerProps {
...
export function VideoModelManager({ searchSpaceId }: VideoModelManagerProps) {
	const t = useTranslations("videoModelSettings");

	// Video gen config atoms
	const {
		mutateAsync: createConfig,
		isPending: isCreating,
		error: createError,
	} = useAtomValue(createVideoGenConfigMutationAtom);
	const {
		mutateAsync: updateConfig,
		isPending: isUpdating,
		error: updateError,
	} = useAtomValue(updateVideoGenConfigMutationAtom);
	const {
		mutateAsync: deleteConfig,
		isPending: isDeleting,
		error: deleteError,
	} = useAtomValue(deleteVideoGenConfigMutationAtom);
	...
	const canCreate = useMemo(() => {
		...
		return access.permissions?.includes("video_generations:create") ?? false;
	}, [access]);
	const canDelete = useMemo(() => {
		if (!access) return false;
		if (access.is_owner) return true;
		return access.permissions?.includes("video_generations:delete") ?? false;
	}, [access]);
	// Backend uses video_generations:create for update as well
	const canUpdate = canCreate;
	const isReadOnly = !canCreate && !canDelete;

	// Local state
	const [isDialogOpen, setIsDialogOpen] = useState(false);
	const [editingConfig, setEditingConfig] = useState<VideoGenerationConfig | null>(null);
	const [configToDelete, setConfigToDelete] = useState<VideoGenerationConfig | null>(null);
		...
		try {
			if (editingConfig) {
				await updateConfig({
					...
					provider: formData.provider as VideoGenerationConfig["provider"],
				...
				if (result?.id) {
					await updatePreferences({
						search_space_id: searchSpaceId,
						data: { video_generation_config_id: result.id },
					});
				}
			}
	...
	const openEditDialog = (config: VideoGenerationConfig) => {
	...
	const selectedProvider = VIDEO_GEN_PROVIDERS.find((p) => p.value === formData.provider);
	const suggestedModels = getVideoGenModelsByProvider(formData.provider);
					...
					{(userConfigs?.length ?? 0) === 0 ? (
						<Card className="border-dashed border-2 border-muted-foreground/25">
							<CardContent className="flex flex-col items-center justify-center py-10 md:py-16 text-center">
								<div className="rounded-full bg-gradient-to-br from-indigo-500/10 to-violet-500/10 p-4 md:p-6 mb-4">
									<Video className="h-8 w-8 md:h-12 md:w-12 text-indigo-600 dark:text-indigo-400" />
								</div>
